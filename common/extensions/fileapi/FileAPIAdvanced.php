 ...
 * ```
 * ...
 * echo $form->field($model, 'file')->widget(FileAPIAdvanced::class, [
 *     'settings' => [
 *         'crop' => true,
 *         'preview' => true
 *     ]
...
class FileAPIAdvanced extends FileAPI
{
    ...
    public function registerClientScript()
    {
        ...
        if ($this->deleteUrl !== null && $this->url) {
            ...
            $view->registerJs(
                "jQuery(document).on('click', '$selector .uploader-delete-current', function(evt) {
				evt.preventDefault();
				var el = jQuery(this);
				jQuery.ajax({
					url : '$deleteUrl',
        ...
        if ($this->deleteTempUrl !== null) {
            ...
            $view->registerJs(
                "jQuery(document).on('click', '$selector .uploader-delete-temp', function(evt) {
				evt.preventDefault();
				var el = jQuery(this),
				    uid = el.data('id'),
        ...
        if ($this->crop !== false) {
            ...
            $this->_defaultSettings['onSelect'] = new JsExpression(
			    ...
			    if(ui.other.length)
			    {
			        ...
			        if(errors)
			        {
			            console.log(ui.other[0]);
			            var message = '';

			            if(errors.minHeight)
			                message = 'Минимальная высота " . $this->settings['imageSize']['minWidth'] . "px';
                        if(errors.minWidth)
			                message += ' Минимальная ширина " . $this->settings['imageSize']['minHeight'] . "px';

			            if(errors.maxHeight)
			                 message += ' Максимальная высота '+errors.maxHeight+'px';
			            if(errors.maxWidth)
			                message += ' Максимальная ширина '+errors.maxWidth+'px';

                        if(errors.maxSize)
			                message += ' Максимальный размер '+errors.maxSize+'мб';

			            if(errors.maxFiles)
			               message += ' Максимальное количество файлов '+errors.maxFiles;
					...
					jQuery('#modal-crop').themodal({
						...
						onOpen: function (overlay) {
			                ...
			                jQuery('.uploader-crop', overlay).cropper({
			                	...
			                	minSize: [$this->cropResizeWidth, $this->cropResizeHeight],
			                	selection: '100%',
